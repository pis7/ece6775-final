...
//----------------------------------------------------------
// Square root function using standard sqrt()
// with return type as ac_fixed
//----------------------------------------------------------

ac_fixed<42, 26, true, AC_RND, AC_SAT> compute_sqrt(ac_fixed<42, 26, true, AC_RND, AC_SAT> input) {
    // Handle negative input by working with the absolute value
    ac_fixed<42, 26, true, AC_RND, AC_SAT> input_abs = (input < (ac_fixed<42, 26, true, AC_RND, AC_SAT>)0.0) ? -input : input;

    // Use standard sqrt from math.h and cast the result to ac_fixed
    double sqrt_result = std::sqrt((double)input_abs.to_double());

    // Return the result as ac_fixed
    return ac_fixed<42, 26, true, AC_RND, AC_SAT>(sqrt_result);
}

//----------------------------------------------------------
// Square root function using Newton's method (Babylonian method)
// with return type as ac_fixed
//----------------------------------------------------------
...
//     // Initialize guess for Newton's method
//     ac_fixed<42, 26, true, AC_RND, AC_SAT> guess = input_abs / 2.0;

//     // Iterate to improve the guess
//     const int max_iterations = 10;  // Limit the number of iterations for convergence
//     for (int i = 0; i < max_iterations; ++i) {
//         // Newton's method iteration: guess = 0.5 * (guess + input / guess)
//         guess = 0.5 * (guess + input_abs / guess);
//     }

//     // Return the final computed square root
//     return guess;
// }
...
void rms_norm(
  ...
  variance = (attn_fixed_t)1.0 / compute_sqrt(variance / C + epsilon);
...
attn_fixed_t attention_round(attn_fixed_t a) {
    attn_fixed_t outf = (a > (attn_fixed_t)0.0) ? (attn_fixed_t)(a + (attn_fixed_t)0.5) : (attn_fixed_t)(a - (attn_fixed_t)0.5);
    return outf;
}
