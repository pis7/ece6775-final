...
attn_fixed_t attention_abs(attn_fixed_t a) {
  return (a < (attn_fixed_t)0.0) ? (attn_fixed_t)(-a) : (attn_fixed_t)a;
}
...
//     double sqrt_result = sqrt((double)input_abs.to_double());

//     // Return the result as ac_fixed
//     return attn_fixed_t(sqrt_result);
// }



// attn_fixed_t compute_sqrt(attn_fixed_t input) {
//     attn_fixed_t input_abs = attention_abs(input);  // attention_abs should return an ac_fixed type
//     ac_fixed<64,32,false> sqrt_result;  // Intermediate result type (make sure it's large enough for the sqrt)
//     sqrt(input_abs, sqrt_result);  // Calls the sqrt function for ac_fixed types
//     return attn_fixed_t(sqrt_result);
// }

// Approximate division using multiplication by the reciprocal
attn_fixed_t compute_sqrt(attn_fixed_t input) {
...
void quantize_activation(
  ...
  QUANTIZE_ACTIVATION_LOOP_1: for (int i = 0; i < R; i++) {
      ...
      QUANTIZE_ACTIVATION_LOOP_3: for (int j = 0; j < C; j++) {
          // sbit32_t quantized_value = (sbit32_t)attention_round(input[i][j] * scale);
          sbit32_t quantized_value = static_cast<sbit32_t>((input[i][j] * scale).to_int());
          // sbit32_t quantized_value = static_cast<sbit32_t>attention_round(input[i][j] * scale);
